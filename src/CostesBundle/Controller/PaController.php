...
class PaController extends Controller
{
	...
	public function editAction(Request $request, $id)
	{
		...
		if ($form->isSubmitted()) {
			try {
				$EM->persist($Pa);
				$EM->flush();
				$params = ["id" => $Pa->getId(), "actuacion" => "UPDATE", "eqpa_id" => 0];
		...
		$params = ["pa" => $Pa, "accion" => "MODIFICACIÓN", "form" => $form->createView()];
	...
	public function addAction(Request $request)
	{
		...
		if ($form->isSubmitted()) {
			...
			try {
				$EM->persist($Pa);
				$EM->flush();
				$params = ["id" => $Pa->getId(), "actuacion" => "INSERT", "eqpa_id" => 0];
				return $this->redirectToRoute("sincroPa", $params);
			} catch (UniqueConstraintViolationException $ex) {
				$status = " YA EXISTE UNA PUNTO ASISTENCIAL CON ESTE CÓDIGO: " . $Pa->getPa();
				$this->sesion->getFlashBag()->add("status", $status);
		...
		$params = ["pa" => $Pa, "accion" => "NUEVO", "form" => $form->createView()];
		return $this->render("costes/pa/edit.html.twig", $params);
	}
	...
	public function queryAction(Request $request)
	{
		...
		return $this->render('costes/pa/query.html.twig', $params);
	}
	 ...
	 * @throws \Exception
	 */
	public function sincroAction($id, $actuacion, $eqpa_id)
	{
		$em = $this->getDoctrine()->getManager();
		...
		$php_script = $php . " " . $root . "/scripts/costes/actualizacionPa.php " . $modo
			. "  " . $Pa->getId() . " " . $actuacion . " " . $eqpa_id;
		...
		$params = ["SincroLog" => $SincroLog,"resultado" => $resultado];
	 ...
	 * @return \Symfony\Component\HttpFoundation\RedirectResponse
	 */
	public function activarAction($eqpa_id)
	{
		$em = $this->getDoctrine()->getManager();
		$EqPa = $em->getRepository("CostesBundle:EqPa")->find($eqpa_id);
		$params = ["id" => $EqPa->getPa()->getId(),
			"actuacion" => 'ACTIVAR',
			"edificio" => $EqPa->getEdificio()->getCodigo(),
			"eqpa_id" => $EqPa->getId()];
		return $this->redirectToRoute("sincroPa", $params);
	}
	 ...
	 * @return \Symfony\Component\HttpFoundation\RedirectResponse
	 */
	public function desActivarAction($eqpa_id)
	{
		$em = $this->getDoctrine()->getManager();
		$EqPa = $em->getRepository("CostesBundle:EqPa")->find($eqpa_id);
		$params = ["id" => $EqPa->getPa()->getId(),
